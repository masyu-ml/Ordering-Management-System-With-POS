...
class JLPOSDashboard(QMainWindow):
    def __init__(self, user_role, user_name, user_id="1"):
        super().__init__()
        self.role = user_role
        self.name = user_name
        self.user_id = str(user_id)  # Store it so we can pass it to other modules
    ...
    def load_modules(self):
        ...
        # 0: POS
        self.pos_view = POSModule()  # Usually stays the same unless it needs specific headers
        ...
        # 2: Inventory (Already updated)
        self.inv_view = InventoryModule(user_id=self.user_id)
        self.content_stack.addWidget(self.inv_view)

        # 3: Expenses (UPDATED)
        self.exp_view = ExpenseModule(user_id=self.user_id)
        self.content_stack.addWidget(self.exp_view)

        # 4: Staff (UPDATED)
        self.staff_view = StaffModule(user_id=self.user_id)
        self.content_stack.addWidget(self.staff_view)

        # 5: Analytics (UPDATED)
        self.analytics_view = AnalyticsModule(user_id=self.user_id)
        self.content_stack.addWidget(self.analytics_view)

        # 6: Administration (UPDATED)
        self.admin_view = AdminModule(user_id=self.user_id)
    ...
    def switch_view(self, index, module_name):
        ...
        if module_name in sensitive_modules:
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                self.auto_refresh_current_tab()  # <-- NEW: Refreshes after unlocking
        else:
            self.content_stack.setCurrentIndex(index)
            self.auto_refresh_current_tab()  # <-- NEW: Refreshes immediately

    # --- NEW HELPER FUNCTION ---
    def auto_refresh_current_tab(self):
        """Automatically calls the refresh function of whatever tab is currently open."""
        current_widget = self.content_stack.currentWidget()

        # The Dashboard checks what kind of UI is open, and triggers its specific refresh command
        if hasattr(current_widget, 'load_database_content'):
            current_widget.load_database_content()  # Used by pos_ui.py
        elif hasattr(current_widget, 'refresh_data'):
            current_widget.refresh_data()  # Used by menu_recipe_ui.py
        elif hasattr(current_widget, 'refresh_table'):
            current_widget.refresh_table()  # Used by inventory_ui.py
